...
4. **Words and recovery document (`66%`)** — show the 24 English words, an embedded desktop PDF preview, and one direct A4 PDF Download action. Require two independent confirmations: all words written down and downloaded PDF safely stored; the PDF confirmation remains unavailable until download. The document includes the exact plaintext vault password; the standalone QR and `.ppxrecovery` do not.
