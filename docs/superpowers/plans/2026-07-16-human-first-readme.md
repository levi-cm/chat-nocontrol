...
**Goal:** Replace the spec-oriented README with a polished, accurate welcome
page for app users and privacy or cryptography enthusiasts.

**Architecture:** Keep `README.md` as the single repository entry point. Lead
with product purpose and safe usage, progressively disclose security and
protocol detail, then place development and reference material at the bottom.

**Tech Stack:** GitHub-flavored Markdown, small centered HTML hero, existing
`logo.png`, repository documentation links, npm documentation checks,
markdownlint.
...
- Modify `README.md`: human-facing product overview, status, usage, security,
  enthusiast details, development setup, and selected documentation links.
- Reference `package.json`: authoritative repository package version and npm
  commands; no change required.
- Reference `docs/implementation-status.md`: current verification and release
  blockers; no change required.
- Reference `docs/user-guide.en.md` and `docs/user-guide.de.md`: detailed user
  instructions; no change required.

### Task 1: Replace the README with the approved human-first structure
...
- [ ] **Step 2: Inspect the rendered hierarchy as plain source**
...
Expected: hero and beta warning appear first; user explanation precedes security,
protocol, development, and reference sections.

### Task 2: Verify content, formatting, and links

**Files:**

- Test: `README.md`
- Test: `docs/superpowers/specs/2026-07-16-human-first-readme-design.md`

- [ ] **Step 1: Verify Markdown style**

Run:

```bash
./node_modules/.bin/markdownlint-cli2 README.md docs/superpowers/specs/2026-07-16-human-first-readme-design.md
```

Expected: `Summary: 0 error(s)`.

- [ ] **Step 2: Verify repository formatting and documentation contracts**
...
Expected: Prettier reports no formatting problems and the documentation contract
prints `Docs contract OK`.

- [ ] **Step 3: Verify every relative README link**
...
const missing = links.filter((target) => !existsSync(resolve(target)));
if (missing.length) {
  console.error(missing.join("\n"));
  process.exit(1);
}
console.log(`${links.length} relative README links resolve.`);
'
```

Expected: all relative README links resolve and command exits `0`.

- [ ] **Step 4: Check version, feature, and stale wording**

Run:

```bash
rg -n "0\.1\.0-beta\.1|PPXQ|256 KiB|100 MiB|independent" README.md
! rg -n "Authority:|repository stays private" README.md
```

Expected: current version, QR, limits, and independent-review warning appear;
`Authority:` and `repository stays private` do not appear.
...
Expected: no whitespace errors; only `README.md` is modified by implementation;
pre-existing untracked files remain untouched.

### Task 3: Commit the README update
...
Expected: staged diff contains only `README.md` with no whitespace errors.

- [ ] **Step 2: Commit the human-first README**

Run:

```bash
git commit -m "docs: redesign readme for people"
```

Expected: commit succeeds without staging `AGENTS.md` or unrelated plan files.
